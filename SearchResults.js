...
import './SearchResults.css'; // optional external styling
...
const SearchResults = () => {
  ...
  // const [products] = useState(search);
  ...
  return (
    <div style={{ padding: '30px' }}>
      ...
      <div className="SearchResults-product-grid">
        {filteredProducts.length > 0 ? (
          filteredProducts.map((product) => (
            <Link to={product.path} key={product.id} className="SearchResults-product-card">
              <img src={product.image} alt={product.name} className="SearchResults-product-img" />
              {/* <h3 className="product-name">{product.name}</h3> */}
              <h4>
                {product.name.split(" ").slice(0, 7).join(" ")}
                {product.name.split(" ").length > 7 && "..."}
              </h4>
              <div className="SearchResults-product-price">
                <span className="SearchResults-price">{product.price}</span>
                <span className="SearchResults-old-price">{product.oldPrice}</span>
              </div>
              {product.sale && <span className="SearchResults-sale-badges">Sale</span>}
            </Link>
          ))
        ) : (
          <p>No items found.</p>
